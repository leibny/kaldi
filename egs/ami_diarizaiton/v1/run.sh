...
if [ $stage -le 2 ]; then
  ...
  sid/train_diag_ubm.sh --cmd "$train_cmd --mem 20G" \
    ...
    data/$mic/train_oraclespk $num_components exp/$mic/diag_ubm_$num_components

  sid/train_full_ubm.sh --nj 40 --remove-low-count-gaussians false \
    --cmd "$train_cmd --mem 25G" --apply-cmn false \
    data/$mic/train_oraclespk exp/$mic/diag_ubm_$num_components \
    exp/$mic/full_ubm_$num_components

  sid/train_ivector_extractor.sh \
    --cmd "$train_cmd --mem 35G" \
    --ivector-dim $ivector_dim --num-iters 5 --apply-cmn false \
    exp/$mic/full_ubm_$num_components/final.ubm data/$mic/train_oraclespk \
    exp/$mic/extractor_c${num_components}_i${ivector_dim}
fi
...
if [ $stage -le 3 ]; then
  ...
  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 25G" \
    ...
    --hard-min true exp/$mic/extractor_c${num_components}_i${ivector_dim} \
  ...
  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 20G" \
    ...
    --min-segment 0.5 exp/$mic/extractor_c${num_components}_i${ivector_dim} \
...
if [ $stage -le 4 ]; then
  ...
  "$train_cmd" exp/$mic/ivectors_dev/log/plda.log \
    ivector-compute-plda ark:exp/$mic/ivectors_train_oraclespk/spk2utt \
      "ark:ivector-subtract-global-mean \
      scp:exp/$mic/ivectors_train_oraclespk/ivector.scp ark:- \
      | transform-vec exp/$mic/ivectors_dev/transform.mat ark:- ark:- \
      | ivector-normalize-length ark:- ark:- |" \
    exp/$mic/ivectors_dev/plda || exit 1;

  # Train a PLDA model on training set, using eval to whiten.
  # We will later use this to score iVectors in dev.
  "$train_cmd" exp/$mic/ivectors_eval/log/plda.log \
    ivector-compute-plda ark:exp/$mic/ivectors_train_oraclespk/spk2utt \
      "ark:ivector-subtract-global-mean \
      scp:exp/$mic/ivectors_train_oraclespk/ivector.scp ark:- \
      | transform-vec exp/$mic/ivectors_eval/transform.mat ark:- ark:- \
      | ivector-normalize-length ark:- ark:- |" \
    exp/$mic/ivectors_eval/plda || exit 1;
fi
...
if [ $stage -le 5 ]; then
  ...
  diarization/score_plda.sh --cmd "$train_cmd --mem 4G" \
    --nj $(($ns<20?$ns:20)) exp/$mic/ivectors_eval exp/$mic/ivectors_dev \
    exp/$mic/ivectors_dev/plda_scores

  # Do the same thing for eval.
  ns=$(cat data/$mic/eval/wav.scp | wc -l)
  diarization/score_plda.sh --cmd "$train_cmd --mem 4G" \
    --nj $(($ns<20?$ns:20)) exp/$mic/ivectors_dev exp/$mic/ivectors_eval \

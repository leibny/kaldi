...
################################
  for name in train callhome1 callhome2 ; do ### callhome1 callhome2 are missing
    steps/segmentation/detect_speech_activity.sh --nj 40 --cmd "$cpu_cmd_eval" --stage 4 \
     --extra-left-context 79 --extra-right-context 21 \
     --extra-left-context-initial 0 --extra-right-context-final 0 \
     --frames-per-chunk 150 --mfcc-config conf/mfcc_hires.conf \
     data/$name exp/segmentation_1a/tdnn_stats_asr_sad_1a \
     mfccdir_vad exp/segmentation_1a/tdnn_stats_asr_sad_1a $name
## I should change the workdir later. Lets first see what is going on.
  done
#################################
#exit

###################################################
#### This part has already been done by the SAD. :)
#  # The sre dataset is a subset of train
#  cp data/train_hires/{feats,vad}.scp data/sre/
...
#  # Reduce the amount of training data for the UBM.
  utils/subset_data_dir.sh train_seg 16000 data/train_16k
  utils/subset_data_dir.sh train_seg 32000 data/train_32k

#  # Train UBM and i-vector extractor.
  sid/train_diag_ubm2.sh --cmd "$train_cmd --mem 20G" \
    --nj 40 --num-threads 8 --delta-order 1 --apply-cmn false \
    data/train_16k $num_components exp/diag_ubm_$num_components

  sid/train_full_ubm2.sh --nj 40 --remove-low-count-gaussians false \
    --cmd "$train_cmd --mem 25G" --apply-cmn false \
    data/train_32k exp/diag_ubm_$num_components \
    exp/full_ubm_$num_components

  sid/train_ivector_extractor2.sh \
    --cmd "$train_cmd --mem 35G" \
    --ivector-dim $ivector_dim --num-iters 5 --apply-cmn false \
    exp/full_ubm_$num_components/final.ubm train_seg \
...
#  # Reduce the amount of training data for the PLDA,
  utils/subset_data_dir.sh train_seg 128000 data/sre_segmented_128k
...
#  # many iVectors for each recording.
  ...
  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 20G" \
    ...
    callhome1_seg exp/ivectors_callhome1

  diarization/extract_ivectors.sh --cmd "$train_cmd --mem 20G" \
    --nj 40 --window 1.5 --period 0.75 --apply-cmn false \
    --min-segment 0.5 exp/extractor_c${num_components}_i${ivector_dim} \
    callhome2_seg exp/ivectors_callhome2
fi
...
if [ $stage -le 7 ]; then
  ...
  diarization/cluster.sh --cmd "$train_cmd --mem 4G" \
    --reco2num-spk data/callhome1_hires/reco2num_spk \
    exp/ivectors_callhome1/plda_scores exp/ivectors_callhome1/plda_scores_num_spk

  diarization/cluster.sh --cmd "$train_cmd --mem 4G" \
    --reco2num-spk data/callhome2_hires/reco2num_spk \
